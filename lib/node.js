...
Node.prototype.startWeb3Connection = async function()
{
	...
	web3 = new Web3(new Web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '4202')));
...
Node.prototype.getStats = async function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		try {
			...
			// Get consensus info only every other update
			let consensusInfo = {
			...
			if (forced || this._lastFetch % (UPDATE_INTERVAL * 2) === 0) {
				try {
					const payload = {
						jsonrpc: "2.0",
			...
			if(results.peers !== null)
			{
				...
				self.stats.gasPrice = results.gasPrice.toString();
